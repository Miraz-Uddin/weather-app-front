...
(function () {
  ...
  const UI = {
    loadingAllNecessarySelectors() {
      ...
      const temperatureIconDisplay = document.querySelector(
        "#temperatureIconDisplay"
      );
      const cityFeelDisplay = document.querySelector("#cityFeelDisplay");
      const cityTemperatureDisplay = document.querySelector(
        "#cityTemperatureDisplay"
      );
      const cityPressureDisplay = document.querySelector(
        "#cityPressureDisplay"
      );
      const cityHumidityDisplay = document.querySelector(
        "#cityHumidityDisplay"
      );
    ...
    getInputValues() {
      const { countryCode, cityNameInput } =
        this.loadingAllNecessarySelectors();
    ...
    displayCurrentWeather(data) {
      ...
      temperatureIconDisplay.setAttribute(
        "src",
        this.getIconSrc(weather[0].icon)
      );
    },
    resetInput() {
      const { countryCode, cityNameInput } =
    ...
    init() {
      ...
      formElem.addEventListener("submit", async (e) => {
        ...
        if (error)
          return this.showMessageInUI("error", "Please Provide Valid Input");
